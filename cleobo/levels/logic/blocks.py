...
def handle_key_blocks(screen, key_block_pairs, player):
    for pair in key_block_pairs:
        # 1. HANDLE THE KEY COLLECTION
        key_x, key_y, key_r, key_color = pair["key"]
        key_rect = pygame.FRect(key_x - key_r, key_y - key_r, key_r * 2, key_r * 2)
        
        if not pair["collected"] and player.rect.colliderect(key_rect):
            pair["collected"] = True
            if not manage_data.is_mute:
                manage_data.sounds['open'].play()

        # 2. HANDLE THE BLOCK COLLISION (Only if key isn't collected)
        if not pair["collected"]:
            ...
            if player.rect.colliderect(block):
                # Vertical Collision
                if player.velocity_y > 0 and player.rect.bottom - player.velocity_y <= block.top:
                    player.rect.bottom = block.top
                    player.velocity_y = 0
                    player.on_ground = True
                elif player.velocity_y < 0 and player.rect.top - player.velocity_y >= block.bottom:
                    player.rect.top = block.bottom
                    player.velocity_y = 0
                
                # Horizontal Collision
                elif player.rect.right > block.left and player.rect.left < block.right:
                    if player.rect.centerx < block.centerx:
